...
public class VideoFrameImageLoader {
    ...
    public List<VideoModelBean> getVideoinfos() {
        return videoinfos;
    }

    public void setVideoinfos(List<VideoModelBean> videoinfos) {
    ...
    public VideoFrameImageLoader(Context context, View listView, List<VideoModelBean> videoinfos) {
        //获取系统分配给每个应用的最大内存，每个应用系统分配32M
        int maxMemory = (int) Runtime.getRuntime().maxMemory();
        Log.e(TAG, "maxMemory = " + maxMemory);
        //给lruCache分配1/8 4M
    ...
    private void showImage(int firstVisibleItem, int visibleItemCount) {
        ...
        for (int i = firstVisibleItem; i < firstVisibleItem + visibleItemCount; i++) {
            ...
            try{
                videoUrl = videoinfos.get(i).getPath();
            }catch (Exception e){
    ...
    private Bitmap getBitmapFormUrl(String url) {
        ...
        bitmap = ThumbnailUtils.createVideoThumbnail(url, MediaStore.Images.Thumbnails.MINI_KIND);
        if(bitmap==null){

        }else{
            bitmap = ThumbnailUtils.extractThumbnail(bitmap, 720, 720, ThumbnailUtils.OPTIONS_RECYCLE_INPUT);
