...
public class NewConsumeRecoadAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        // 消费金额
        String consumeNumber = consumeRecoadList.get(position).getConsumeNumber();
        if (DataUtil.isNullorEmpty(consumeNumber)) {
            viewholder.new_consume_money.setText("未知消费");
        } else {
            if (consumeNumber.startsWith("-")) {
                String refund = consumeNumber.replaceFirst("-", "+");
                viewholder.new_consume_money.setText(refund);
            } else {
                viewholder.new_consume_money.setText("-" + consumeNumber);
            }
        }
        ...
        String consumeType = consumeRecoadList.get(position).getConsumeType();
