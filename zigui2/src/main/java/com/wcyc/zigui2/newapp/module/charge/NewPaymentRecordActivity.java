...
import com.wcyc.zigui2.newapp.adapter.NewPaymentRecordsAdapter;
...
public class NewPaymentRecordActivity extends BaseActivity 
	...
	private void GetOrderList() {
		...
		new HttpRequestAsyncTask(json, this, this).execute(Constants.GET_ORDER_LIST);
	}
	...
	private void getOrderDetail(String orderId){
		...
		new HttpRequestAsyncTask(json,this,this).execute(Constants.GET_ORDER_DETAIL);
	}
	...
	private void initEvents() {
		...
		lv_payment_records.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				OrderDetail detail = (OrderDetail) arg0.getAdapter().getItem(arg2);
...
//					payPop.showAsDropDown(arg1);
	...
	private void initDatas() {
		...
		mPaymentRecordsAdapter = new NewPaymentRecordsAdapter(this, detailList,list);
	...
	public void onRequstComplete(String data) {
		List<OrderInfo> myOrderResultList;
		if(data.contains("orderList")){
			...
			if (mOrderListResult.getServerResult().getResultCode() != 200) {
				...
				myOrderResultList = mOrderListResult.getOrderList();
				int size = 0;
				if(myOrderResultList != null && (size = myOrderResultList.size()) > 0){
					list.addAll(myOrderResultList);
					for(int i = 0; i < size; i++){
						long orderId = myOrderResultList.get(i).getId();
						getOrderDetail(orderId+"");
					}
				}
			}
		}else{
			mOrderDetailList = JsonUtils.fromJson(data, NewOrderDetailList.class);
			if(mOrderDetailList.getServerResult().getResultCode() == Constants.SUCCESS_CODE){
				List<OrderDetail> list = mOrderDetailList.getOrderDetail();
				if(list != null && list.size() > 0){
					payment_record_ll_null.setVisibility(View.GONE);
					detailList.add(list.get(0));
					mPaymentRecordsAdapter.notifyDataSetChanged();
				}
			}
		}
	}
	...
	protected void getMessage(String data) {
		...
		if(ret.getServerResult().getResultCode() == Constants.SUCCESS_CODE){
			...
			OrderDetail detail = detailList.get(pos);
			UserType user = CCApplication.getInstance().getPresentUser();
			long actual = detail.getAmount();
			long origin = detail.getFullAmount();
			req.setUserId(user.getUserId());
			req.setOrderId(detail.getOrderId()+"");
			req.setStudentId(user.getChildId());
			req.setCouponAmount(origin-actual);
			req.setCouponType("0");
			req.setFullAmount(origin);
			req.setOriginalPayAmount(actual);
			req.setPayNo(detail.getId()+"");
			req.setPayAmount(actual);
			req.setPayDate(detail.getStartDate());
			req.setPayType(BUY);
	...
	private void getPaymentInfo(){
		JSONObject json = new JSONObject();
		try {
			json.put("orderId", detailList.get(pos).getOrderId()+"");
