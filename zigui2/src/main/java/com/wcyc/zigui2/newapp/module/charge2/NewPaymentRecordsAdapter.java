...
public class NewPaymentRecordsAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		ViewHolder holder;
//		if(convertView == null){
			convertView = inflater.inflate(R.layout.new_payment_record_item2, parent , false);
...
//		}
	...
	private void initDataToView(ViewHolder holder, int position) {
		final ChargeLogs mRenewOrder = list.get(position);
		...
		long amount = mRenewOrder.getOrderAmount();
...
//		holder.tv_renew_fee_duration.setText(mRenewOrder.getRechargeTime()+"个月");
		holder.tv_renew_fee.setText(mRenewOrder.getProductName());
		holder.tv_record_ordernum.setText(mRenewOrder.getOrderNo()+"");
		holder.tv_record_ordertime.setText(mRenewOrder.getRechargeTime());
		holder.payment_start_stop_tv.setText(mRenewOrder.getCreateTime());
	}
