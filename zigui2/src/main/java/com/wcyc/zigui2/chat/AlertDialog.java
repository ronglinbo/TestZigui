...
//		voicePath = getIntent().getStringExtra("voicePath");
		...
		if(isCanceShow){
			...
			mButton.setBackgroundResource(R.drawable.cancel_btn_select);
			confirm_btn.setBackgroundResource(R.drawable.confirm_btn_select);
		}

		if(path != null){
			 //优先拿大图，没有去取缩略图
			if(!new File(path).exists())
				path = getThumbnailImagePath(path);
