...
public class GroupListAdapter extends BaseAdapter {
    ...
    public void expandNode(TreeNode root){
        alls.clear();
        if(root != null){
            ...
            for(TreeNode item : list){
                alls.add(item);
                if(item.isExpanded()) {
                    List<TreeNode> child = item.getChildren();
                    if(child != null){
                        for(TreeNode childItem : child) {
                            alls.add(childItem);
                            if(childItem.isExpanded()){
                                List<TreeNode> leaf = childItem.getChildren();
                                if(leaf != null){
                                    alls.addAll(leaf);
                                }
                            }
     ...
     * @param position
     */
    public boolean ExpandOrCollapse(TreeNode root,int position) {
       ...
       /*     if (isOffline(n)) {
            ...
            if (n.getType() == 1) {
                ...
                if (n.isExpanded() /*|| n.isLeaf()*/) {
                    holder.ivExEc.setImageResource(R.drawable.icon_xiangshagn);
...
//                    holder.ivExEc.setVisibility(View.GONE);
                ...
                // 设备不在线则不显示选择框
                if (n.getDeviceInfo() != null && n.getDeviceInfo().getStatus() != 1) {
                    holder.chbSelect.setVisibility(View.GONE);
                }
            	...
            	if ( n.getChannelInfo() != null && n.getChannelInfo().getState() != 1) {
            		holder.chbSelect.setVisibility(View.GONE);
            	}else {
            	    holder.chbSelect.setVisibility(View.VISIBLE);
            	}
            ...
            // 控制缩进
             if (n.getValue().equals("...")) {
                ...
                view.setPadding(30 * (n.getLevel()-1), 3, 3, 3);
             }
            // 设置不同层级节点的item颜色
    ...
    private boolean isOffline(TreeNode n) {
        boolean ret = false;
        if (n.getType() == 2 && n.getDeviceInfo() != null && n.getDeviceInfo().getStatus() != 1) {
            ret = true;
